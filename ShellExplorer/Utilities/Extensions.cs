...
namespace ShellExplorer.Utilities
{
    internal static class Extensions
    {
        public static HRESULT AddToBindCtx(this IBindCtx bindContext, string name, object value, bool throwOnError = true)
        {
            ...
            var hr = EnsureBindCtxPropertyBag(bindContext, out var ps, throwOnError);
            if (hr != 0)
                return hr;
        ...
        public static HRESULT EnsureBindCtxPropertyBag(this IBindCtx bindContext, out object? store, bool throwOnError = true)
        {
            ...
            if (unk == null)
            {
                var hr = Methods.PSCreateMemoryPropertyStore(typeof(IPropertyStore).GUID, out var ps).ThrowOnError(throwOnError);
                if (hr != 0)
                    return hr;
